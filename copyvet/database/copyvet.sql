...
-- VISTAS
-- ======================================================
-- 🔹 VISTA: Listado de Veterinarios
CREATE
OR REPLACE VIEW vista_veterinarios AS
SELECT
...
-- 🔹 VISTA: Detalle de Veterinario
CREATE
OR REPLACE VIEW vista_detalle_veterinario AS
SELECT
    u.id_usuario AS id_veterinario,
    u.nombre_completo,
    u.correo,
    ...
    SUM(
        CASE
            WHEN e.nombre_estado = 'Cerrado' THEN 1
            ELSE 0
        END
    ) AS tickets_cerrados,
    SUM(
        CASE
            WHEN e.nombre_estado = 'En proceso' THEN 1
            ELSE 0
        END
    ) AS tickets_en_proceso,
    SUM(
        CASE
            WHEN e.nombre_estado IN ('Abierto', 'En proceso') THEN CASE
                WHEN s.descripcion = 'Urgente' THEN 4
                WHEN s.descripcion = 'Alta Prioridad' THEN 3
                WHEN s.descripcion = 'Normal' THEN 2
                WHEN s.descripcion = 'Baja Prioridad' THEN 1
                ELSE 0
            END
            ELSE 0
        END
    ) AS carga_trabajo_prioridad,
    COUNT(
        CASE
            WHEN e.nombre_estado IN ('Abierto', 'En proceso')
            AND TIMESTAMPDIFF (MINUTE, NOW (), t.fecha_cita) <= s.tiempo_resolucion THEN 1
        END
    ) AS tickets_proximos_vencer
FROM
    usuarios u
    LEFT JOIN tickets t ON u.id_usuario = t.id_asignado_a_usuario
    LEFT JOIN estadosticket e ON e.id_estado = t.id_estado
...
CREATE
OR REPLACE VIEW vista_categorias AS
SELECT
    c.id_categoria,
    c.nombre_categoria,
...
CREATE
OR REPLACE VIEW vista_detalle_categoria AS
SELECT
    c.id_categoria,
    c.nombre_categoria,
    s.descripcion AS prioridad,
    s.tiempo_minutos AS tiempo_respuesta,
    s.tiempo_resolucion AS tiempo_resolucion,
    COUNT(t.id_ticket) AS total_tickets,
    SUM(
        CASE
            WHEN e.nombre_estado = 'Cerrado' THEN 1
...
CREATE
OR REPLACE VIEW vista_tickets AS
SELECT
    t.id_ticket,
    t.titulo,
    t.descripcion,
    ...
    CASE
        WHEN TIMESTAMPDIFF (MINUTE, t.fecha_creacion, NOW ()) <= s.tiempo_minutos THEN 'Cumple'
        ELSE 'No Cumple'
    END AS cumplimiento_respuesta,
    CASE
        WHEN e.nombre_estado = 'Cerrado'
        AND TIMESTAMPDIFF (MINUTE, t.fecha_creacion, NOW ()) <= s.tiempo_resolucion THEN 'Cumple'
        WHEN e.nombre_estado != 'Cerrado'
        AND TIMESTAMPDIFF (MINUTE, NOW (), t.fecha_cita) > s.tiempo_resolucion THEN 'No Cumple'
        ELSE 'En Progreso'
...
-- ======================================================
-- CONSULTAS SEGÚN ROL (para usar en la aplicación)
-- ======================================================
...
-- 🔸 VETERINARIO: Ver solo tickets asignados a él
SELECT
    *
FROM
    vista_tickets
